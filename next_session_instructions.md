...
> Poslední aktualizace: 2026-05-23 (UCAF v1.3.3 live + TD-H14 RESOLVED + docs commitnuty)

## Kde jsme skončili

- **UCAF v1.3.3 pushed** na `honzamalak-afk/ucaf` (HEAD `d37cbd1`). Unity repo manifest + lock repinned, package reload + recompile verified, listener back online v 1.3.3 protocol_capabilities.
- **TD-H14 RESOLVED.** `generate_sprite` má nyní `alpha_method` dispatch:
  - `edge_color` (default) — sample 1px border → median bg color → flood s Chebyshev tolerance. Live regression "potion bottle icon" = **85.5 % alpha (224k px), 1 attempt, 7054 ms** (předtím 0 % alpha v1.3.2).
  - `floodfill` (legacy) — white-only z corners. Live regression na stejném prompt = **0 % alpha, 2 attempts** (auto-retry kicked in correctly).
  - `rembg` (opt-in) — `python -m rembg i` subprocess. Async event-based readers (no pipe-fill deadlock). Vyžaduje `pip install rembg[cli]` + ~200 MB ONNX na first run. Path netestována — Honza ještě nedoručil `python --version; pip show rembg` output.
...
- **alpha_method=rembg path netestována live.** Implementation hotová + async-safe + graceful error pokud python/rembg missing. Otestuje se buď když Honza pošle Python+rembg ověření, nebo až bude reálný sprite case kde edge_color selže (full-frame subject, hair/glass detail).
...
- Pokud chceš rembg path otestovat: `pip install rembg[cli]` na Windows side, pak send_command s `alpha_method=rembg`

## Důležitý kontext / pozor na

- **UCAF v1.3.3 HEAD** `d37cbd1b2f8ccba11bbc1c9a9795f66dffc7e077`. Listener answering, 160 commands enumerated, package_version reported correctly.
...
## Files touched v této session (2026-05-23 TD-H14 fix)

UCAF dev repo (`/workspace/_ucaf_dev/`, pushed jako `d37cbd1`):
